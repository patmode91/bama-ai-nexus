...
class MCPAgentCurator {
  ...
  async enrichBusinessData(sessionId: string, businesses: any[], context: MCPContext): Promise<CuratorResponse> {
    try {
      ...
      const enrichedBusinesses = await Promise.all(
        businesses.map(business => this.enrichSingleBusiness(business, context))
      );

      const dataQualityReport = this.generateDataQualityReport(enrichedBusinesses);
      const suggestions = this.generateDataSuggestions(enrichedBusinesses, context);
  ...
  private async enrichSingleBusiness(business: any, context: MCPContext): Promise<EnrichedBusinessData> {
  ...
  private generateDataQualityReport(enrichedBusinesses: EnrichedBusinessData[]) {
  ...
  private generateDataSuggestions(enrichedBusinesses: EnrichedBusinessData[], context: MCPContext): string[] {
